...
def generate_adv(x, label, network, eps=0.01):
    ...
    adv = adv.reshape(1, 28, 28)
    return adv

f_train = []

for x, t in zip(x_train, t_train):
    f_train.append(generate_adv(x, t, network, 0.3))

f_train = np.array(f_train)

# print(f_train.shape)

trainer = Trainer(network, f_train, t_train, x_test, t_test,
...
# パラメータの保存
network.save_params("deep_convnet_params_fgsm.pkl")
